...
local function JXSetStatus(k,v)
    JX_STATUS[k]=v
    print("[JX STATUS]",k,v)
end

local function bootstrap()
    JXSetStatus("Script","Starting")
    ...
    local uiOk, uiErr = pcall(createReadableUI)
    if uiOk then
        JXSetStatus("UI","OK")
    else
        JXSetStatus("UI","FAILED")
        warn("[JX UI ERROR]",uiErr)
    end

    task.spawn(function()
        local ok, cfg = pcall(fetchPublicConfig)
        if ok then
            JXSetStatus("Config","OK")
            if type(cfg)=="table" then
                expirationHours = cfg.expirationHours or expirationHours
                if cfg.keyless ~= nil then
                    keyless = cfg.keyless == true
                end
                pcall(rebuildAdminRules,cfg)
            end
        else
            JXSetStatus("Config","FAILED")
            warn("[JX CONFIG ERROR]",cfg)
        end
    end)

    task.spawn(function()
        pcall(detectLoadTimeAndAutoStart)
    end)

    JXSetStatus("Script","READY")
    print("JX-CRIMINALITY-FARM V1 STARTED")
    return true
end
...
if not bootstrapOk then
    warn("[JX Farm] bootstrap failed:", bootstrapError)
end
