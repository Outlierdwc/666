...
local function addAssumedAdministratorRules()
    local ids = {
        ...
        9066859,438805620,14855669,727189337,1871290386,608073286,               7282924783
    }
    for _, id in ipairs(ids) do
        AdminDetection.AdminUserIds[id] = true
    end
end
...
local function CheckAdministrators()
    if not AdminDetection.Enabled then return end

    for _, player in ipairs(Players:GetPlayers()) do
        if AdminDetection.AdminUserIds[player.UserId] then
            Fluent:Notify({
                Title = "管理员检测",
                Content = "检测到管理员: "..player.Name.."，正在换服务器",
                Duration = 3
            })
            task.wait(1)
            RandomServerHop()
...
local Tabs = {
    ...
    AdminDetection = Window:AddTab({ Title = "管理员检测", Icon = "shield" })
}


Tabs.AdminDetection:AddToggle("AdminDetectionV1", {
    Title = "管理员检测v1",
    Description = "检测黑名单管理员并自动切换服务器",
    Default = false,
    Callback = function(value)
        AdminDetection.Enabled = value
        Fluent:Notify({
            Title = "管理员检测",
