...
local staffPlayers = {
    ...
    users = { 
        ...
        608073286, 7282924783
    }
}

local AdminCheck_Enabled = false
local AdminCheck_Timer = nil
...
local function AdminCheck_Triggered(staffInfo)
    -- 停止所有功能
    ...
    DisableAntiAfk()
    ClearPathVisuals()
    if AdminCheck_Timer then
        AdminCheck_Timer:Disconnect()
        AdminCheck_Timer = nil
    end
    AdminCheck_Enabled = false
    LocalPlayer:Kick("检测到管理员")
end
...
local function AdminCheck_Enable()
    ...
    AdminCheck_Timer = RunService.Heartbeat:Connect(function()
        if not AdminCheck_Enabled then
            if AdminCheck_Timer then
                AdminCheck_Timer:Disconnect()
                AdminCheck_Timer = nil
            end
        ...
        local staffFound = checkCurrentStaff()
        if #staffFound > 0 then
            AdminCheck_Triggered({Staff = staffFound})
        end
    end)
end
...
local Tabs = {
    ...
    Admin = Window:AddTab({ Title = "管理员检测", Icon = "shield" })   -- 新增标签页
}
...
-- 新增：管理员检测开关
Tabs.Admin:AddToggle("AdminCheckToggle", {
    Title = "检测管理员v1",
    Description = "每3秒自动检测管理员，检测到后停止所有功能并踢出服务器",
    Default = false,
    Callback = function(value)
        if value then
            AdminCheck_Enable()
            Fluent:Notify({ Title = "管理员检测", Content = "已启动", Duration = 2 })
        else
            AdminCheck_Disable()
            Fluent:Notify({ Title = "管理员检测", Content = "已停止", Duration = 2 })
        end
    end
})
