...
-- ZYZ.lua stability patch
-- Added executor compatibility layer and protected startup flow.
...
if Request then
    Environment.request = Environment.request or Request
end

local function safeCall(name, callback)
    local ok, result = pcall(callback)
    if not ok then
        warn("[JX Farm] " .. name .. " failed:", result)
    end
    return ok, result
end
