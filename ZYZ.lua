...
local function randomServerHop()
    ...
    if not ok then
        Log("Server hop failed: "..tostring(result))
    end
end

local function startAdminDetector()
    if AdminDetectThread then return end
    AdminDetectThread = task.spawn(function()
        while AdminDetectEnabled do
...
local Tabs = {
    ...
    Admin = Window:AddTab({ Title = "管理员检测", Icon = "shield" })
}
...
Tabs.Admin:AddToggle("AdminDetectToggle", {
    Title = "自动检测并通知v1",
    Description = "检测白名单管理员后自动随机换服务器",
    Default = false,
    Callback = function(value)
