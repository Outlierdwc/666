-- ============================================================
-- JX Criminality Farm (保留管理员检查, 移除密钥验证)
-- 完整版, 请进入游戏世界后执行
-- ============================================================
...
local WINDOW_NAME = "JX | Criminality | FARM"
...
local function safeRead(path)
    if type(readfile) ~= "function" or not safeIsFile(path) then return nil end
...
local function addAdministratorRole(groupId, roleName)
    ...
    if not roles then roles = {}; adminGroupRoles[groupId] = roles end
    roles[roleName] = true
end
...
local function serializeRuntimeState()
    return "EarnMoney:" .. tostring(math.floor(earnedMoneyTotal))
end
...
-- ====== 隐身 ======
local invisHeartbeatConnection, invisCharacterConnection
local invisWarningGui, invisWarningLabel, invisAnimation, invisTrack

local function ensureInvisWarningGui()
    if invisWarningGui and invisWarningGui.Parent then return end
    local existing = UiParent:FindFirstChild("JXInvisWarningGUI") or UiParent:FindFirstChild("InvisWarningGUI")
    if existing then
...
local function ensureInvisHeartbeat()
    ...
    invisHeartbeatConnection = RunService.Heartbeat:Connect(function()
        if invisEnabled then pcall(updateInvisCharacter)
        elseif invisWarningLabel then invisWarningLabel.Visible = false end
    end)
end

local function invisEnable()
    local character = getCharacter()
    if not character or not character:FindFirstChild("Torso") then return false end
    userWantsInvis = true
    invisEnabled = true
...
local function shouldRejoinFromError(message)
    ...
    for _, fragment in ipairs({"kicked","disconnect","connection","error code","same account","teleport failed","server shut","shutdown"}) do
...
local function installAntiRejoin()
    ...
    antiRejoinConnections[#antiRejoinConnections + 1] = GuiService.ErrorMessageChanged:Connect(function(message)
...
local function collectMoneyTarget(target)
    ...
    local moved = pcall(function() root.CFrame = target.part.CFrame + Vector3.new(0, 2, 0) end)
    if moved then markMove(); waitSeconds(0.10) end
...
local function classifyTargetZone(model, part)
    ...
    if lowerName:find("sw11", 1, true) or (pos - SW11_SECOND_POSITION).Magnitude < 450 then return "SW11" end
    if lowerName:find("tower", 1, true) or (pos - TOWER_FIRST_POSITION).Magnitude < 450 then return "Tower" end
    if lowerName:find("su", 1, true) or (pos - SU_FIRST_POSITION).Magnitude < 500 or (pos - SU_LOW_POSITION).Magnitude < 500 then return "SU" end
    return "Normal"
end
...
local function findCandidateTargets()
    ...
    for _, container in ipairs(containers) do
        for _, object in ipairs(container:GetDescendants()) do
            if object:IsA("Model") and not seen[object] and not isTemporarilyIgnored(object) and not processedTargets[object] then
...
local function tweenRootTo(position, targetCFrame)
    ...
    local completed, playbackState = false
    local connection = tween.Completed:Connect(function(state) playbackState = state; completed = true end)
    tween:Play()
    local startedAt = now()
    ...
    if breakingMethod == "Crowbar" then
        ...
        while farmEnabled and target.Parent and not targetIsBroken(target) and now() - startedAt < 120 do
            ...
            if opened then
                ...
                while target.Parent and not targetIsBroken(target) and now() - completedAt < 12 do
                    waitSeconds(0.10)
                end
                break
            end
...
local function performDepositRequest(events, cash)
    ...
    if not moveToPart(atmMainPart) then return false end
...
local function isLikelyAdmin(player)
    ...
    for groupId, roles in pairs(adminGroupRoles) do
        ...
        if ok and roles[tostring(roleName)] then return true end
    end
    for groupId, minimumRank in pairs(adminGroupRanks) do
        local ok, rank = pcall(player.GetRankInGroup, player, groupId)
        if ok and type(rank) == "number" and rank >= minimumRank then return true end
    end
    return false
end
        ...
        if adminCheckEnabled then
            for _, player in ipairs(Players:GetPlayers()) do
                if isLikelyAdmin(player) then
                    ...
                    showNotification("JX", "Administrator detected: " .. player.Name, 5)
                    return
                end
            end
        end
...
-- ============================================================
-- 辅助功能：AntiAfk, Allowance, AutoPlay, 余额读取
-- ============================================================
...
local function readCashAmountValue()
    ...
    local object = PlayerGui:FindFirstChild("CoreGUI")
    if object then
        for _, name in ipairs({"Cash","CashLabel","CashAmount","Money","MoneyLabel"}) do
            local child = object:FindFirstChild(name, true)
            if child then
                local ok, val = pcall(function()
                    if child:IsA("TextLabel") or child:IsA("TextButton") or child:IsA("TextBox") then
                        return child.Text
                    elseif child:IsA("NumberValue") or child:IsA("IntValue") or child:IsA("StringValue") then
                        return child.Value
                    end
                    return child.Text or child.Value
                end)
                if ok then
                    return parseCashTextToNumber(val)
                end
            end
        end
    end
...
local function detectLoadTimeAndAutoStart()
    ...
    if autoPlayLoadTimeDetected and autoPlayEnabled then
        ...
        while now() < autoPlayLoadTimeReadyAt do waitSeconds(0.1) end
        userWantsFarm = true
        if not farmEnabled then startFarm() end
    end
end
...
local function startFarm()
    ...
    spawnTask(function() adminWatchLoop(runId) end)  -- 管理员检查
    spawnTask(function()
        while farmEnabled and farmRunId == runId do
...
local function createReadableUI()
    ...
    createUiElement("UIListLayout", { Padding = UDim.new(0, 6), SortOrder = Enum.SortOrder.LayoutOrder }, scrolling)
    ...
    makeToggle(scrolling, "Admin Check", "JXFarmAdminCheck", adminCheckEnabled)   -- 保留管理员检查开关
...
local function setFlag(name, value)
    if name == "JXFarmEnabled" then
        ...
        return true
    elseif name == "JXFarmAutoRespawn" then autoRespawn = value == true
    elseif name == "JXFarmAutoPlay" then setAutoPlay(value)
    elseif name == "JXFarmAutoDeposit" then setAutoDepositEnabled(value)
    elseif name == "JXFarmAutoMoney" then autoMoney = value == true
    elseif name == "JXFarmAntiRejoin" then
        antiRejoin = value == true
        if antiRejoin then installAntiRejoin() end
    elseif name == "JXFarmAntiAfk" then setAntiAfk(value)
    elseif name == "JXFarmAdminCheck" then adminCheckEnabled = value == true
    ...
    elseif name == "CharacterAntiFallDamage" then setNoFall(value)
    elseif name == "JXFarmAutoDepositThresholdK" then setAutoDepositThresholdK(value)
...
local function bootstrap()
    ...
    deferTask(function()
        ...
        if not ok then warn("[JX Farm] UI creation failed:", err) end
    end)
    print("JX-CRIMINALITY-FARM FULLY LOADED (no verification, admin check retained)")
    return true
end

local bootOk, bootErr = bootstrap()
if not bootOk then
    warn("[JX Farm] bootstrap failed:", bootErr)
end