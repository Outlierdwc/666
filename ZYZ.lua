...
-- ========== [ADDED] Auto Admin Detect (移植自 Criminality) ==========
local AdminDetectEnabled = false
local adminCheckLoopRunId = 0
local adminUserIds = {}
...
-- ========== [ADDED] Admin Detect Tab ==========
local AdminTab = Window:AddTab({ Title = "管理员检测", Icon = "shield" })

AdminTab:AddToggle("AdminDetectToggle", {
    Title = "检测管理员v1",
    Description = "每隔3秒检测服务器管理员，检测到后自动停止所有功能并踢出服务器",
    Default = false,
    Callback = function(value)
        AdminDetectEnabled = value
        if value then
            adminCheckLoopRunId = adminCheckLoopRunId + 1
            local runId = adminCheckLoopRunId
            task.spawn(function()
                while AdminDetectEnabled and adminCheckLoopRunId == runId do
                    for _, player in ipairs(Players:GetPlayers()) do
                        if isLikelyAdmin(player) then
                            Fluent:Notify({ Title = "管理员检测", Content = "检测到管理员: " .. player.Name, Duration = 5 })
                            Settings.Enabled = false
                            if AutoPickupRunning then
                                StopAutoPickup()
                            end
                            local kickOk, _ = pcall(function()
