...
-- Admin detection (ported from Criminality新.lua)
local AdminCheck = {
    Enabled = false,
    NotifyV1 = false,
    LastDetected = {}
...
local function notifyV1(message)
    pcall(function()
...
local function checkAdmins()
    if not AdminCheck.Enabled then
        return
    end

    for _, player in ipairs(Players:GetPlayers()) do
        if isLikelyAdmin(player) and not AdminCheck.LastDetected[player.UserId] then
            AdminCheck.LastDetected[player.UserId] = true

            if AdminCheck.NotifyV1 then
                notifyV1("检测到白名单管理员: " .. player.Name)
            end

            task.spawn(randomServerHop)
            return
        end
    end
end
