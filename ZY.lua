...
local function claimAllowance()
    ...
    local ok, accepted, _, _, amount = pcall(function()
        return remote:InvokeServer(atm)
    end)
...
local function tryDeposit()
    if not autoDepositEnabled or depositInProgress then
        return false
    end
    local now = tick()
    if now < depositCooldownUntil or now - depositLastAttemptAt < 1.5 then
        return false
    end
    ...
    local ok, result = pcall(function()
        local accepted = remote:InvokeServer("DP", cash, atm)
        return accepted == true
    end)
    depositInProgress = false
    depositCooldownUntil = tick() + 2.5
    return ok and result
end

local function setAutoDeposit(value)
