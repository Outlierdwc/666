...
-- Added features from Criminality新.lua
local AutoAllowanceEnabled = false
local AutoDepositEnabled = false
local DepositThreshold = 5000
local DepositBusy = false
...
local function BuildNewWaypointPath(fromPosition, toPosition)
    local path = PathfindingService:CreatePath({
        WaypointSpacing = Settings.WaypointSpacing,
    })
    ...
    return #points > 0 and points or {toPosition}
end
...
local function MoveToTarget(targetPart)
    ...
    if not hrp or not humanoid or not targetPart or not targetPart:IsA("BasePart") then
        return false
    end
    ...
    local targetPos = GetPositionInFrontOfTarget(targetPart, hrp.Position)
    if not targetPos then
        IsMovingToTarget = false
        return false
    end

    local points = BuildNewWaypointPath(hrp.Position, targetPos)
    ...
    for _, position in ipairs(points) do
        ...
        if distance > 0.2 then
            local tween = TweenService:Create(
                hrp,
                TweenInfo.new(distance / math.max(1, Settings.MoveSpeed), Enum.EasingStyle.Linear),
...
-- Auto allowance + ATM deposit from Criminality新.lua
local function ReadMoneyAndAllowance()
    ...
    local function read(name)
        local obj = stats:FindFirstChild(name, true)
        local value = obj and (obj.Text or obj.Value)
        return tonumber(tostring(value or ""):gsub("[^%d]", "")) or 0
    end

    return read("Bank"), read("Allowance")
end

local function ClaimAllowance()
    if not AutoAllowanceEnabled then return end
    local events = ReplicatedStorage:FindFirstChild("Events")
    local remote = events and events:FindFirstChild("Allowance")
...
local function DepositATM()
    if not AutoDepositEnabled or DepositBusy then return end
    local bank, _ = ReadMoneyAndAllowance()
    if bank < DepositThreshold then return end

    local map = workspace:FindFirstChild("Map")
    local atm = map and map:FindFirstChild("ATMz")
    local model = atm and atm:FindFirstChild("ATM")
    local part = model and model:FindFirstChild("MainPart")
    local events = ReplicatedStorage:FindFirstChild("Events")
    local remote = events and events:FindFirstChild("ATM")

    if not part or not remote or not remote:IsA("RemoteFunction") then return end

    DepositBusy = true
    pcall(function()
        remote:InvokeServer("DP", bank, part)
    end)
    DepositBusy = false
end
...
Tabs.Main:AddToggle("AutoAllowanceToggle", {
    ...
    Default = false,
    Callback = function(value)
        AutoAllowanceEnabled = value
    end
})

Tabs.Main:AddToggle("AutoDepositToggle", {
    Title = "ATM Auto Deposit",
    Default = false,
    Callback = function(value)
