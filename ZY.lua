...
if not PlayerGui then
    warn("[JX Farm] PlayerGui not ready, falling back to CoreGui")
end
...
local function bootstrap()
    print("[JX Farm] Bootstrap started")
    ...
    if not ok then
        warn("[JX Farm] Initial setup failed:", err)
    end

    task.spawn(function()
        local configOk, publicConfig = pcall(fetchPublicConfig)

        if configOk and type(publicConfig) == "table" then
            expirationHours =
            ...
            pcall(rebuildAdminRules, publicConfig)
        else
            warn("[JX Farm] Public config unavailable")
        end
    end)

    task.spawn(function()
        local ok, err = pcall(detectLoadTimeAndAutoStart)

        if not ok then
            warn("[JX Farm] Auto start failed:", err)
        end
    end)
