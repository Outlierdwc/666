...
local AutoDepositThreshold = 5000
...
local function ClaimAllowance()
    ...
    local atm = FindATM()
    if remote and atm then
        pcall(function()
            remote:InvokeServer(atm)
        end)
    end
end
...
local function AutoDeposit()
    ...
    if GetPlayerCash() < AutoDepositThreshold then return end
    ...
    DepositBusy = true
    pcall(function()
        remote:InvokeServer("DP", GetPlayerCash(), atm)
    end)
    DepositBusy = false
end
...
local function MainFarmLoop()
    ...
    while true do
        ...
        Log("=== Цикл фермы ===")
        if AutoAllowanceEnabled then pcall(ClaimAllowance) end
        if AutoDepositEnabled then pcall(AutoDeposit) end
        local character = LocalPlayer.Character
        local humanoid = character and character:FindFirstChildOfClass("Humanoid")
...
Tabs.Main:AddSlider("AutoDepositThresholdSlider", {
    ...
    Rounding = 0,
    Callback = function(value)
        AutoDepositThreshold = value * 1000
    end
})
