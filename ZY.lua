...
local 
-- ===== Imported from Criminality新.lua: Auto Allowance + ATM Deposit =====
local AllowanceAmount = 0
local BankAmount = 0
local DepositCooldown = 0
...
local function ReadStatsGui()
    ...
    AllowanceAmount = parse(stats:FindFirstChild("Allowance", true)) or AllowanceAmount
    BankAmount = parse(stats:FindFirstChild("Bank", true)) or BankAmount
end
...
local function ClaimAllowance()
    ...
    local ok, accepted, msg, blocked, amount = pcall(
        remote.InvokeServer,
        remote,
        part
    )
...
local function MoveToATM(part)
    ...
    local tween = TweenService:Create(
        root,
        TweenInfo.new(math.max(.1,distance/Settings.MoveSpeed), Enum.EasingStyle.Linear),
...
local function GetCashAmount()
    ...
    local cash = stats:FindFirstChild("Cash", true)
    if not cash then return 0 end

    return tonumber(tostring(cash.Text or ""):gsub("[^%d]", "")) or 0
end

local function AutoDepositATM()
    if not Settings.AutoDeposit or DepositBusy then return false end
    if os.clock() < DepositCooldown then return false end

    local cash = GetCashAmount()
    if cash < Settings.DepositThreshold then return false end

    local events = ReplicatedStorage:FindFirstChild("Events")
    local remote = events and events:FindFirstChild("ATM")
    local atm = FindATMPart()
    ...
    local success = false
    pcall(function()
        if MoveToATM(atm) then
            local ok = remote:InvokeServer("DP", cash, atm)
            success = ok == true
        end
    end)

    DepositBusy = false
    DepositCooldown = os.clock()+3
